...
let logURL = NSURL(string: "https://raw.githubusercontent.com/jmoenig/Snap--Build-Your-Own-Blocks/archive/master/history.txt")
...
public func shouldUpdate() -> Bool{
    let dictNew: NSDictionary = NSDictionary(object: NSDate(), forKey: updateKey)
    let logFile: NSData = NSKeyedArchiver.archivedDataWithRootObject(dictNew)
    logFile.writeToFile(lastUpdatePath, atomically: true)
    
    if !compareHistory() || !fileManager.fileExistsAtPath(lastUpdatePath){
        return true
    }
...
private func compareHistory() -> Bool{
    let newHistory = String(contentsOfURL: logURL!, encoding: NSUTF8StringEncoding, error: nil)
    let oldHistoryPath = getSnapPath().stringByAppendingPathComponent("history.txt")
    if(!fileManager.fileExistsAtPath(oldHistoryPath)){
        return false
    }
    let oldHistory = String(contentsOfFile: oldHistoryPath, encoding: NSUTF8StringEncoding, error: nil)
