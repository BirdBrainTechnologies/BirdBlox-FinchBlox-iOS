//
//  HummingbirdBitUtilities.swift
//  BirdBlox
//
//  Created by birdbrain on 5/8/2018.
//  Copyright © 2017 Birdbrain Technologies LLC. All rights reserved.
//
//
// This file provides utility functions for interacting with the HummingbirdBit
//

import Foundation

class BBTHummingbirdBitUtility {
	/**
    ...
    /**
     Gets a command to set the buzzer on the hummingbird
     */
    static public func getBuzzerCommand(volume: UInt8, frequency: UInt8) -> Data {
        let letter: UInt8 = 0x53
        return Data(bytes: UnsafePointer<UInt8>([letter, volume, frequency] as [UInt8]), count: 3)
    }
    
    /**
     Gets a command to set the LED array on the Microbit
     */
    static public func getLEDArrayCommand(_ ledArray: [UInt8]) -> Data {
        let letter: UInt8 = 0x53
        var realIntensities: [UInt8] = []
        for i in 0 ..< ledArray.count {
            let bounded_intensity = bound(ledArray[i], min: 0, max: 100)
            realIntensities[i] = UInt8(floor(Double(bounded_intensity)*2.55))
        }
        return Data(bytes: UnsafePointer<UInt8>([letter] + realIntensities as [UInt8]), count: ledArray.count + 1)
    }
