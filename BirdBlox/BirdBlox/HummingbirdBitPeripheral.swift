//
//  HummingbirdBitPeripheral.swift
//  BirdBlox
//
//  Modified version of HummingbirdPeripheral.swift
//  created on 4/25/2018 by Kristina Lauwers.
...
class HummingbirdBitPeripheral: NSObject, CBPeripheralDelegate, BBTRobotBLEPeripheral {
	public let peripheral: CBPeripheral
	public var id: String {
		return peripheral.identifier.uuidString
	}
	
	public static let type: BBTRobotType = .HummingbirdBit
...
//	var commandMode = false
    //TODO: Should some of this text change?
	...
	required init(peripheral: CBPeripheral, completion: ((BBTRobotBLEPeripheral) -> Void)? = nil){
        ...
        self.peripheral.discoverServices([HummingbirdBitPeripheral.SERVICE_UUID])
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        if let services = peripheral.services{
            for service in services {
                if(service.uuid == HummingbirdBitPeripheral.SERVICE_UUID){
                    peripheral.discoverCharacteristics([HummingbirdBitPeripheral.RX_UUID,
                                                        HummingbirdBitPeripheral.TX_UUID],
														for: service)
                    return
                }
            }
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        if let characteristics = service.characteristics{
            for characteristic in characteristics {
                if(characteristic.uuid == HummingbirdBitPeripheral.TX_UUID){
                    tx_line = characteristic
                    peripheral.setNotifyValue(true, for: characteristic )
                    wasTXSet = true
                }
                else if(characteristic.uuid == HummingbirdBitPeripheral.RX_UUID){
...
//		DispatchQueue.main.async{
		if self.useSetall {
			...
			Timer.scheduledTimer(timeInterval: self.syncInterval, target: self,
			                     selector: #selector(HummingbirdBitPeripheral.syncronizeOutputs),
...
//		print("Did update characteristic \(characteristic)")
        ...
        if characteristic.uuid != HummingbirdBitPeripheral.RX_UUID {
			return
        }
        ...
        inData.copyBytes(to: &self.lastSensorUpdate, count: HummingbirdBitPeripheral.sensorByteCount)
    }
