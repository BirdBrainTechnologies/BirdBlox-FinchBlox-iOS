//
//  HummingbirdDuoPeripheral.swift
//  BirdBlox
//
...
class HummingbirdDuoPeripheral: NSObject, CBPeripheralDelegate, BBTRobotBLEPeripheral {
	public let peripheral: CBPeripheral
	public var id: String {
		return peripheral.identifier.uuidString
	}
	
	public static let type: BBTRobotType = .HummingbirdDuo
...
//	var commandMode = false
	...
	required init(peripheral: CBPeripheral, completion: ((BBTRobotBLEPeripheral) -> Void)? = nil){
        ...
        self.peripheral.discoverServices([HummingbirdDuoPeripheral.SERVICE_UUID])
    }
    ...
    func peripheral(_ peripheral: CBPeripheral, didDiscoverServices error: Error?) {
        ...
        if let services = peripheral.services{
            for service in services {
                if(service.uuid == HummingbirdDuoPeripheral.SERVICE_UUID){
                    peripheral.discoverCharacteristics([HummingbirdDuoPeripheral.RX_UUID,
                                                        HummingbirdDuoPeripheral.TX_UUID],
														for: service)
                    return
                }
            }
    ...
    func peripheral(_ peripheral: CBPeripheral,
        ...
        if let characteristics = service.characteristics{
            for characteristic in characteristics {
                if(characteristic.uuid == HummingbirdDuoPeripheral.TX_UUID){
                    tx_line = characteristic
                    peripheral.setNotifyValue(true, for: characteristic )
                    wasTXSet = true
                }
                else if(characteristic.uuid == HummingbirdDuoPeripheral.RX_UUID){
...
//		DispatchQueue.main.async{
		if self.useSetall {
			...
			Timer.scheduledTimer(timeInterval: self.syncInterval, target: self,
			                     selector: #selector(HummingbirdDuoPeripheral.syncronizeOutputs),
...
//		print("Did update characteristic \(characteristic)")
        ...
        if characteristic.uuid != HummingbirdDuoPeripheral.RX_UUID {
			return
        }
        ...
        inData.copyBytes(to: &self.lastSensorUpdate, count: HummingbirdDuoPeripheral.sensorByteCount)
    }
